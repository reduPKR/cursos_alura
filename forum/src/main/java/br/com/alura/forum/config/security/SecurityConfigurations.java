...
public class SecurityConfigurations extends WebSecurityConfigurerAdapter {
    ...
    @Autowired
    private TokenService tokenService;//para passar para o filter
    @Autowired
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers(HttpMethod.DELETE, "/topicos/*").hasRole("MODERADOR")//Somente quem for adm pode excluir algo
                .anyRequest().authenticated()
                .and().csrf().disable()
                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and().addFilterBefore(new AutenticacaoViaTokenFilter(tokenService, usuarioRepository), UsernamePasswordAuthenticationFilter.class);//Só é nescessario pois estou fazendo filter para validar o token
                //.and().formLogin(); removido para nao criar sessao
    }

    //Configuração de recursos estaticos (js, css, imagens ...)
    //Usei para liberar as rotas para o swagger
    @Override
